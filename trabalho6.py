...
def home():
    """
    A rota inicial que renderiza a página HTML inicial.

    :return: O template HTML para a página inicial.
    """
...
def schedule():
    """
    Cria um agendamento de turnos de trabalho com base das informações fornecida.

    As informações incluem o número de trabalhadores, o número de turnos por dia, o número de dias no ano e a data de início.
    As informações são recebidos como dados do formulário a partir de uma solicitação POST.

    :return: Uma mensagem indicando que o agendamento  foi criada com sucesso.
    """
