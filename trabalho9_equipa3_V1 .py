...
def schedule():
    ...
    for worker in range(num_workers):
        ...
        for day in range(num_days_year):
            starting_shift = worker % num_shifts_day
            cycle_day = (day + starting_shift * 4) % 28  # now the cycle lasts 28 days
            if cycle_day < 6:  # Morning shift
                shift = 0
            elif 6 <= cycle_day < 9 or 15 <= cycle_day < 18:  # Day off
                shift = -1
            ...
            is_day_off = cycle_day in [6, 7, 8, 15, 16, 17, 24, 25, 26, 27]  
